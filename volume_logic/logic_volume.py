...
EXCEL_SOURCE = DIR.parent / "6_0_BSE_1000_Sector_Allocation___Results_Schedule.xlsx"
MASTER_FILE = DIR.parent / "company_master.json"
...
def load_bse2000_universe():
    ...
    try:
        ...
        name_map = {}    # Sym -> Name
        bse_map = {}     # Sym -> BSE Code
        valid_symbols = []
        
        for _, row in df.iterrows():
            sym = str(row[11]).strip().upper()  # NSE Symbol (Col L)
            name = str(row[2]).strip()          # Company Name (Col C)
            bse = str(row[1]).strip()           # BSE Code (Col B)
            subgroup = str(row[9]).strip()      # Industry (Col J)
...
def render(template_name, output_name, data):
    ...
    with open(DIR.parent / output_name, "w", encoding="utf-8") as f: f.write(html)
